// PDF Generation Module for Staff Reports using pdfmake
// This file handles PDF generation for staff forms

function generateStaffPDF() {
    // Get form data
    const formData = {
        userType: 'staff', // Hardcoded for staff report
        fullName: document.getElementById('staffFullName')?.value || '',
        subject: document.getElementById('staffSubject')?.value || '',
        place: document.getElementById('staffPlace')?.value || '',
        age: document.getElementById('staffAge')?.value || '',
        reportNotes: document.getElementById('staffReportNotes')?.value || '',
    ...
    // Create PDF document
    createStaffPDFDocument(formData, signatures);
}
...
function createStaffPDFDocument(formData, signatures) {
    // Format dates
    ...
    const witnessSignature = signatures.witness || '';
    const mainSignature = signatures.staff || '';
    const witnessName = formData.witnessName;
    const mainPosition = formData.staffType;
    const witnessPosition = formData.witnessPosition;
        ...
        .then(([logoDataUrl, witnessSignatureDataUrl, mainSignatureDataUrl]) => {
            ...
            const docDefinition = {
                                    ...
                                    stack: [
                                        ...
                                        },
                                        {
                                            text: (() => {
                                                ...
                                                if (formData.staffOffice) {
                                                    textArray.push(formData.staffOffice + '\n');
                                                }
                ...
                content: [
                    ...
                    // Document Title - Staff specific
                    {
                        text: 'STATEMENT OF A STAFF',
                        fontSize: 12,
                        bold: true,
                        alignment: 'center',
                        margin: [0, 0, 0, 15]
                    ...
                    // Name Section
                    {
                        table: {
                            ...
                            body: [
                                [
                                    { text: 'Name:', bold: true, margin: [5, 5, 0, 5], border: [true, true, false, false] },
                                    { text: formData.fullName, margin: [0, 5, 5, 5], border: [false, true, true, false] }
                                ],
                                [
                                    { text: 'Position:', bold: true, margin: [5, 5, 0, 5], border: [true, false, false, false] },
                                    { text: formData.staffType, margin: [0, 5, 5, 5], border: [false, false, true, false] }
                                ],
                                [
                                    { text: 'Office:', bold: true, margin: [5, 5, 0, 5], border: [true, false, false, true] },
                                    { text: formData.staffOffice, margin: [0, 5, 5, 5], border: [false, false, true, true] }
                                ]
                            ]
                        },
                        layout: {
            ...
            // Generate filename
            const filename = `Staff_Statement_${formData.fullName.replace(/\s+/g, '_')}_${new Date().toISOString().slice(0, 10)}.pdf`;
